...
RESULT_ROOT="${RESULT_ROOT:-${ROOT_DIR}/benchmark/results/remote_guardrails_single_gpu_v0}"
CLEAN_PATH="${CLEAN_PATH:-benchmark/data/clean/v0-clean-core.jsonl}"
RISK_PATH="${RISK_PATH:-benchmark/data/risk/v0-risk-core.jsonl}"
REWRITE_PATH="${REWRITE_PATH:-benchmark/data/attack_rewrite/v0-attack-rewrite.jsonl}"
SLEEP_BETWEEN_MODELS="${SLEEP_BETWEEN_MODELS:-10}"
STARTUP_RETRIES="${STARTUP_RETRIES:-120}"
STARTUP_SLEEP_SECONDS="${STARTUP_SLEEP_SECONDS:-10}"
MODELS="${MODELS:-granite_guardian_3_3_8b,llama_guard_3_8b,qwen2_5_7b_instruct,llama_guard_4_12b,qwen2_5_14b_instruct}"
STATUS_FILE="${STATUS_FILE:-${ROOT_DIR}/server/guardrail_eval_status.txt}"
...
run_one_model() {
  ...
  local output_dir="${RESULT_ROOT}/${model_name}"
  mkdir -p "${output_dir}"

  local progress_bar
  progress_bar="$(render_bar "${model_index}" "${total_models}")"
  echo "== ${progress_bar} Starting ${model_name} =="
  echo "Log: ${log_path}"
  echo "Datasets: clean=${CLEAN_PATH} risk=${RISK_PATH} rewrite=${REWRITE_PATH}"
  ...
  write_status "${progress_bar} | model=${model_name} | phase=running_benchmark"
  (
    cd "${ROOT_DIR}"
    python benchmark/scripts/run_remote_guardrail_baselines.py \
      --server-ip "${SERVER_IP}" \
      --api-key "${API_KEY}" \
      --models "${model_name}" \
      --clean "${CLEAN_PATH}" \
  ...
  echo "== ${progress_bar} Finished ${model_name} =="
  write_status "${progress_bar} | model=${model_name} | phase=completed | output=${output_dir}"
  cleanup
  SERVER_PID=""
...
for idx in "${!MODEL_LIST[@]}"; do
  ...
  run_one_model "${model_name}" "$((idx + 1))" "${TOTAL_MODELS}"
done
